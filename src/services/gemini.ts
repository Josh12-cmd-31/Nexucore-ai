...
export async function generateNexuCoreResponse(
  ...
  const modeInstructions = {
    ...
    pdf: "FOCUS: PDF Tools Mode. You are an expert document formatter. Your goal is to help users prepare structured text for PDF export. Provide clear, well-organized content that will look professional when converted to a PDF document.",
    ...
    code: "FOCUS: Code Assistant Mode. You are an elite software architect and full-stack developer. Provide high-performance, secure, and scalable code solutions. Focus on: 1. Clean Code principles, 2. Design Patterns, 3. Security best practices, 4. Performance optimization. Explain the 'why' behind architectural decisions.",
    general: ""
  }[mode as keyof typeof modeInstructions] || "";

  const model = "gemini-3-flash-preview";

  const userText = `[PERSONA: ${persona.toUpperCase()}] ${personaInstruction}\n\n[MODE: ${mode.toUpperCase()}] ${modeInstructions}\n\n${message}`;
